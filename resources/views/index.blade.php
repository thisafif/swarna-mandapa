@php
    ...
    // ── Hero ──────────────────────────────────────────────────────────────────
    $heroTitle      = HomeContent::get('hero_title',       'Swarna Mandapa');
    $heroSubtitle   = HomeContent::get('hero_subtitle',    'A golden sanctuary where tradition and luxury live in perfect harmony.');
    $heroButtonText = HomeContent::get('hero_button_text', 'Check Availability →');
    $heroVideoUrl   = HomeContent::get('hero_video_url',   '');
    $heroImageUrl   = HomeContent::get('hero_image_url',   '');

    // ── About Section ─────────────────────────────────────────────────────────
    $aboutHeritageImg = HomeContent::get('about_heritage_img', asset('assets/swarna/about-heritage.png'));
    $aboutGardenImg   = HomeContent::get('about_garden_img',   asset('assets/swarna/about-garden.png'));

    // ── Amenities ─────────────────────────────────────────────────────────────
    $amenitiesImg = HomeContent::get('amenities_img', asset('assets/swarna/amenities.png'));

    // ── Gallery Preview ───────────────────────────────────────────────────────
    $galleryLivingImg = HomeContent::get('gallery_living_img', asset('assets/swarna/gallery-living.png'));
    $galleryRoomImg   = HomeContent::get('gallery_room_img',   asset('assets/swarna/gallery-room.png'));
    $galleryDoorImg   = HomeContent::get('gallery_door_img',   asset('assets/swarna/gallery-door.png'));
    $galleryKitchenImg = HomeContent::get('gallery_kitchen_img', asset('assets/swarna/gallery-kitchen.png'));
    $galleryStatueImg = HomeContent::get('gallery_statue_img', asset('assets/swarna/gallery-statue.png'));
    $galleryPoolImg   = HomeContent::get('gallery_pool_img',   asset('assets/swarna/gallery-pool.png'));

    // ── Suites ────────────────────────────────────────────────────────────────
    $masterSuiteImg   = HomeContent::get('master_suite_img',    asset('assets/swarna/master-suite.png'));
    $grandSuiteImg    = HomeContent::get('grand_suite_img',     asset('assets/swarna/grand-suite.png'));
    $guestSuite1Img   = HomeContent::get('guest_suite_1_img',   asset('assets/swarna/guest-suite-1.png'));
    $guestSuite2Img   = HomeContent::get('guest_suite_2_img',   asset('assets/swarna/guest-suite-2.png'));
    $guestSuite3Img   = HomeContent::get('guest_suite_3_img',   asset('assets/swarna/guest-suite-3.png'));
...
<x-layout title="Swarna Mandapa">
    ...
    <main class="overflow-hidden">
        ...
        <section id="home" class="relative flex min-h-screen flex-col items-center justify-center text-white pt-[80px] sm:pt-[96px]">
            ...
            @elseif($heroImageUrl)
                <img src="{{ $heroImageUrl }}" class="absolute inset-0 size-full object-cover brightness-65" alt="Swarna Mandapa">
            @else
                <video class="absolute inset-0 size-full object-cover brightness-65" autoplay muted loop playsinline aria-label="Swarna Mandapa pool courtyard">
                    <source src="{{ asset('assets/swarna/sm.MP4') }}" type="video/mp4">
                </video>
            @endif
            ...
            <div class="relative z-10 grid w-full justify-items-center px-5 pb-12 text-center sm:px-12 lg:pb-[72px]" data-reveal>
                <h1 class="font-brand text-[44px] font-medium leading-tight text-[#ffdc7d] sm:text-6xl lg:text-[64px]">{{ $heroTitle }}</h1>
                <p class="mt-3 max-w-4xl text-lg leading-snug sm:text-2xl">{{ $heroSubtitle }}</p>
                <div class="mt-6 grid w-full max-w-[360px] gap-1 px-4">
                    <x-gold-button href="{{ route('booking.form') }}" class="w-full">{{ $heroButtonText }}</x-gold-button>
            ...
            <div class="grid items-center gap-10 lg:grid-cols-2 lg:gap-20" data-parallax>
                <article class="grid gap-10" data-reveal>
                    ...
                    <div class="overflow-hidden">
                        <img src="{{ $aboutHeritageImg }}" alt="Interior courtyard with pool and carved columns" class="h-[420px] w-full object-cover sm:h-[560px] lg:h-[702px]" loading="lazy" data-parallax-item data-parallax-speed="72">
                    </div>
                </article>
                <article class="grid gap-10 lg:mt-[102px]" data-reveal data-reveal-delay="120">
                    <div class="overflow-hidden">
                        <img src="{{ $aboutGardenImg }}" alt="Tropical garden beside the villa" class="h-[340px] w-full object-cover sm:h-[466px]" loading="lazy" data-parallax-item data-parallax-speed="-56">
                    </div>
        ...
        <section id="features" class="grid items-center gap-10 border border-[#e4dcc8] bg-[#fefdf9] px-5 py-12 sm:px-10 lg:grid-cols-2 lg:gap-0 lg:px-[300px] lg:py-[72px]">
            ...
            <div class="lg:p-2.5" data-reveal data-reveal-delay="160">
                <img src="{{ $amenitiesImg }}" alt="Coffee service and villa kitchen detail" class="h-[420px] w-full object-cover sm:h-[560px] lg:h-[721px]" loading="lazy">
            </div>
        </section>
        ...
        <section id="gallery" class="bg-white px-5 py-14 sm:px-10 lg:px-[300px] lg:py-[108px]">
            ...
            <div class="mx-auto grid max-w-[1296px] gap-3 sm:gap-6 lg:grid-cols-[minmax(0,818px)_minmax(320px,450px)] lg:gap-x-7">
                <div class="grid gap-3 sm:gap-6">
                    <x-image-card :src="$galleryLivingImg" alt="Bright living room with traditional ceiling" class="h-[420px] sm:h-[620px] lg:h-[757px]" />
                    <div class="grid gap-3 sm:grid-cols-2 sm:gap-6">
                        <x-image-card :src="$galleryRoomImg" alt="Guest bedroom" class="h-[260px] sm:h-[365px]" delay="80" />
                        <x-image-card :src="$galleryDoorImg" alt="Carved Balinese door" class="h-[260px] sm:h-[365px]" delay="160" />
                    </div>
                </div>
                <div class="grid gap-3 sm:gap-6">
                    <x-image-card :src="$galleryKitchenImg" alt="Warm kitchen detail" class="h-[180px] sm:h-[197px]" delay="100" />
                    <x-image-card :src="$galleryStatueImg" alt="Carved decorative statue" class="h-[420px] sm:h-[689px]" delay="180" />
                    <x-image-card :src="$galleryPoolImg" alt="Pool and terrace detail" class="h-[180px] sm:h-[212px]" delay="260" />
                </div>
            </div>
        ...
        <section id="suites" class="border border-[#e4dcc8] bg-[#fefdf9] px-5 py-14 sm:px-10 lg:px-[300px] lg:py-[108px]">
            ...
            <div class="grid gap-6 lg:grid-cols-2" data-reveal>
                <div class="relative rounded-2xl overflow-hidden group">
                    <img src="{{ $masterSuiteImg }}" alt="Grand master suite" class="h-[260px] w-full object-cover sm:h-[357px]" loading="lazy">
                ...
                <div class="relative rounded-2xl overflow-hidden group">
                    <img src="{{ $grandSuiteImg }}" alt="Guest suite with carved details" class="h-[260px] w-full object-cover sm:h-[357px]" loading="lazy">
                    <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent flex flex-col justify-end p-6 text-white">
            ...
            <div class="grid gap-4 md:grid-cols-3">
                <x-image-card :src="$guestSuite1Img" alt="Guest suite bed" class="h-[238px]" />
                <x-image-card :src="$guestSuite2Img" alt="Guest suite lounge detail" class="h-[238px]" delay="100" />
                <x-image-card :src="$guestSuite3Img" alt="Guest suite bathroom detail" class="h-[238px]" delay="200" />
            </div>
        </section>
        ...
        <section id="reviews" class="bg-white px-5 py-14 sm:px-10 lg:px-[300px] lg:pb-[144px]">
            ...
            <div class="columns-1 gap-7 md:columns-2 xl:columns-3">
                @foreach ($testimonials as $testimonial)
                    <x-testimonial-card :testimonial="['name' => $testimonial->first_name . ' ' . $testimonial->last_name, 'text' => $testimonial->review]" :delay="$loop->index * 60" />
                @endforeach
            </div>
        </section>
    ...
    <script>
        ...
        function toggleAmenities() {
            ...
            if (!isExpanded) {
                ...
                moreFeatures.style.gap = '2.25rem';
                setTimeout(() => { moreFeatures.style.opacity = '1'; }, 10);
                button.textContent = 'View Less ↑';
                isExpanded = true;
                setTimeout(() => { moreFeatures.scrollIntoView({ behavior: 'smooth', block: 'start' }); }, 100);
            } else {
                moreFeatures.style.opacity = '0';
                setTimeout(() => { moreFeatures.style.display = 'none'; }, 300);
